...
**Execution model.** The GGUF model runs **in-process** inside the Rust backend via the `llama-cpp-4` binding to llama.cpp — no separate inference server, no external process, and no network calls. This keeps all note generation fully on-device, satisfying the zero-egress requirement (NFR-6).

> **Linked, not reachable.** llama.cpp's `common` library is built because the MTP speculative session needs it (§8.2), and `common` carries llama.cpp's own model downloader on top of a vendored cpp-httplib. So an HTTP client is *linked into* the note generator even though nothing calls it — weights come over `ureq` (§8.2), never that path. The conscious call: TLS is compiled out (`LLAMA_OPENSSL=OFF`, `src-tauri/cmake/llama-no-openssl.cmake`), leaving a plain-HTTP client with no caller and no OpenSSL dependency, rather than the default build in which `llama-common.dll` links the host's libcrypto/libssl. Dropping `common` outright is not available — the speculative shim's implementation lives there.
...
**Dependency & fallback.** Reuse needs the `llama-cpp-4` binding to expose **sequence-scoped state save/restore** (`state_seq_get_size_ext` / `state_seq_get_data_ext` / `state_seq_set_data_ext`), confirmed present in the pinned version (0.7.0). The sequence-scoped variants (over the whole-context `get_state_size` / `copy_state_data`) are what keep the snapshot sized to the prefix's cells rather than the full N_CTX cache. The binding *also* exposes KV-cache trim (`clear_kv_cache_seq`), which the persistent-context alternative would have used — but the save/restore path is what we build (see the trade-off below). The **fallback** is the current behavior — prefill the full prompt per note — and it is entered automatically whenever the snapshot is missing (not yet primed, or after `unload`) or the boundary check fails, so the feature degrades to "correct but not cached" rather than breaking generation.
...
```
prefix_kv_<gguf file name>_<sha256 of prompt::prefix()[..8]>_<llama-cpp-sys-4 version>.bin
```
...
- **The version string comes from `Cargo.lock`, read by `build.rs` at compile time** and baked into the binary. It must not be hand-maintained and must not come from `Cargo.toml`: the manifest carries a **range** (`"0.7.0"` means `>=0.7.0, <0.8.0`), so it can drift from what is compiled. Only the lockfile states what is actually compiled in. It cannot be read at runtime because it is a source file and is not shipped in the installer.
