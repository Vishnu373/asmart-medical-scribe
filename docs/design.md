...
**Always falls back.** Every failure — no draft file, a vocabulary that does not match, a context that will not build, a decode error, a refused KV trim — switches the feature off for the session and lets the existing decode loop write the note as it does today. Setting `MEDSCRIBE_NO_SPECULATIVE` does the same on demand, for benchmarking and support — presence is what counts, so any value but `0` or empty turns the feature off ([`setup.md`](setup.md#environment-variables)). Speculative decoding can make a note faster; it can never make one wrong, and it can never turn a working consult into an error.
