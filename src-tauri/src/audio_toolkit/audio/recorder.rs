        ...
        let worker = std::thread::spawn(move || {
            ...
            match init_result {
                Ok((stream, sample_rate)) => {
                    ...
                    run_consumer(sample_rate, sample_rx, cmd_rx, frame_cb, level_cb, stop_flag);
                    drop(stream);
                }
                Err(error_message) => {
...
fn run_consumer(
    ...
    loop {
        ...
        // ---------- resample to 16 kHz and deliver frames ---------------- //
        frame_resampler.push(&raw, |frame: &[f32]| {
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            match cmd {
                ...
                Cmd::Stop(reply_tx) => {
                    ...
                    loop {
                        match sample_rx.recv_timeout(Duration::from_secs(2)) {
                            Ok(AudioChunk::Samples(remaining)) => {
                                frame_resampler.push(&remaining, |frame: &[f32]| {
                                    handle_frame(frame, true, &frame_cb, &mut processed_samples)
                                });
                            }
                    ...
                    frame_resampler.finish(|frame: &[f32]| {
                        handle_frame(frame, true, &frame_cb, &mut processed_samples)
                    });
