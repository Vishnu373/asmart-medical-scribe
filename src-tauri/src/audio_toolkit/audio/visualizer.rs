...
/// Turns raw audio into a small set of normalised spectrum buckets (0..1) for an
/// input-level meter. Vocal-band focused, with a slowly adapting noise floor.
...
impl AudioVisualiser {
    ...
    pub fn feed(&mut self, samples: &[f32]) -> Option<Vec<f32>> {
        ...
        // Drop only the window we consumed; keep the remainder so a cpal chunk
        // larger than window_size (common at 48 kHz) and partial tails carry
        // across calls instead of being discarded.
